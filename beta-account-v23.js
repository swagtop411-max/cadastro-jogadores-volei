...
const twa=/^android-app:\/\//i.test(String(document.referrer||""));
const active=localStorage.getItem("bd_public_beta_v19")==="1"||params.get("beta")==="1"||params.get("app")==="1"||standalone||twa;
if(!active)throw new Error("Conta Mobile inativa");
...
function ensureCss(){if(document.getElementById("betaAccountV23Css"))return;const link=document.createElement("link");link.id="betaAccountV23Css";link.rel="stylesheet";link.href="/beta-account-v23.css?v=20260908-40";document.head.appendChild(link)}
ensureCss();
...
async function handleForcedAccountFlow(user){
 ...
 try{await signOut(auth)}catch(error){console.warn("Conta Mobile: não foi possível encerrar a sessão anterior",error)}
 const target=forceRegister?"/conta.html?tab=register&app=1&fresh=1":"/conta.html?tab=login&app=1&fresh=1";
 location.replace(target);
 return true;
}

function accountLinks(user){
 if(user)return `<a class="beta-account-btn" href="/conta.html?tab=login&app=1&forceLogin=1">TROCAR CONTA</a><a class="beta-account-btn primary" href="/conta.html?tab=register&app=1&forceRegister=1">CRIAR NOVA CONTA</a><a class="beta-account-btn danger" href="/exclusao-conta.html?app=1">EXCLUIR CONTA</a>`;
 return `<a class="beta-account-btn" href="/conta.html?tab=login&app=1">ENTRAR</a><a class="beta-account-btn primary" href="/conta.html?tab=register&app=1">CRIAR CONTA</a>`;
}
...
function syncBetaPanel(user){
 ...
 section.innerHTML=`<strong>CONTA</strong><span>${user?"Você está conectado. Gerencie seu acesso e seus dados.":"Entre ou crie sua conta para participar da rede."}</span><div class="beta-account-panel-actions">${accountLinks(user)}</div>`;
 return true;
}
