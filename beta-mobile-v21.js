...
function boot(){buildTop();mountNav();refresh();onAuthStateChanged(auth,syncAccountState);const feed=document.getElementById('homeFeed');if(feed){new MutationObserver(schedule).observe(feed,{childList:true,subtree:true});new MutationObserver(schedule).observe(document.body,{attributes:true,subtree:true,attributeFilter:['hidden']})}setTimeout(refresh,350);setTimeout(refresh,1200)}
