...
function getHistoricoCampeonatosFromForm(){return [...document.querySelectorAll("#campeonatosLista .campeonato-row")].map(row=>({campeonato:row.querySelector(".campeonato-nome")?.value.trim()||"",colocacao:row.querySelector(".campeonato-colocacao")?.value.trim()||"",ano:row.querySelector(".campeonato-ano")?.value.trim()||""})).filter(x=>x.campeonato||x.colocacao||x.ano)}
