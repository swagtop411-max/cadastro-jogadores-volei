...
function normalizeLocation(cidade,uf){
  ...
  const m=city.match(/^([A-Z]{2})\\s*-\\s*(.+)$/i);
  if(m){
...
onAuthStateChanged(auth,async u=>{if(!u){location.href="conta.html?tab=login&return=/meu-perfil.html";return}user=u;try{const s=await getDoc(doc(db,"perfis",u.uid));profile=s.exists()?s.data():null;const legadoSnap=await getDocs(query(collection(db,"atletas"),where("ownerUid","==",u.uid)));if(!legadoSnap.empty){const legado={id:legadoSnap.docs[0].id,...legadoSnap.docs[0].data()};const legadoLoc=normalizeLocation(legado.cidade,legado.uf);const legadoProfile={uid:u.uid,nome:legado.nome||"",nascimento:legado.nascimento||"",cidade:legadoLoc.cidade||"",uf:legadoLoc.uf||"",modalidade:legado.modalidade||"",posicao:legado.posicao||"",categoria:legado.categoria||"Iniciante",time:legado.time||"",contato:legado.contato||"",bio:legado.bio||legado.observacoes||"",fotoUrl:legado.foto||"",plano:legado.plano||"Gratuito",planoId:legado.planoId||"gratuito",valorPlano:Number(legado.valorPlano||0),planoStatus:legado.planoStatus||"ativo",pagamentoConfirmado:legado.pagamentoConfirmado===true,email:u.email||legado.ownerEmail||"",status:legado.status||"ativo"};await setDoc(doc(db,"perfis",u.uid),legadoProfile,{merge:true});const atualizado=await getDoc(doc(db,"perfis",u.uid));profile=atualizado.exists()?atualizado.data():legadoProfile;}fill();await loadClaimableProfiles();await loadMedia();const params=new URLSearchParams(location.search);const complete=Boolean(profile?.nome&&profile?.cidade&&profile?.uf);if(complete&&!params.has("editar")){location.replace("perfil-social.html?uid="+encodeURIComponent(user.uid))}}catch(e){console.error("Falha ao carregar/sincronizar perfil:",e);fill();await loadClaimableProfiles();await loadMedia()}});