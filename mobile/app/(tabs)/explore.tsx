...
type ExploreFilter = "Todos" | "Praia" | "Quadra" | "Iniciante" | "Intermediário" | "Avançado" | "Completos";
type ExploreSort = "Nome" | "Cidade" | "Categoria";
...
export default function ExploreScreen() {
  ...
  const [sort, setSort] = useState<ExploreSort>("Nome");
  ...
  const filtered = useMemo(() => {
    ...
    const matched = profiles.filter((profile) => {
      ...
      if (filter === "Completos") return profile.completo === true;
      return norm(profile.categoria).includes(norm(filter));
    });

    return [...matched].sort((a, b) => {
      if (sort === "Cidade") {
        return `${a.cidade} ${a.uf} ${a.nome}`.localeCompare(`${b.cidade} ${b.uf} ${b.nome}`, "pt-BR");
      }
      if (sort === "Categoria") {
        return `${a.categoria} ${a.nome}`.localeCompare(`${b.categoria} ${b.nome}`, "pt-BR");
      }
      return a.nome.localeCompare(b.nome, "pt-BR");
    });
  }, [filter, profiles, search, sort]);

  const cities = useMemo(() => new Set(profiles.map((item) => norm(`${item.cidade}-${item.uf}`)).filter(Boolean)).size, [profiles]);
  const teams = useMemo(() => new Set(profiles.map((item) => norm(item.time)).filter(Boolean)).size, [profiles]);
      ...
      ListHeaderComponent={
        <View>
          ...
          <View style={styles.statsRow}>
            <MiniStat value={String(profiles.length)} label="ATLETAS" />
            <MiniStat value={String(cities)} label="CIDADES" />
            <MiniStat value={String(teams)} label="EQUIPES" gold />
          </View>
          ...
          <View style={styles.searchWrap}>
            ...
            <TextInput
              ...
              placeholder="Buscar atleta, cidade, equipe…"
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filterScroll}>
            {(["Todos", "Praia", "Quadra", "Iniciante", "Intermediário", "Avançado", "Completos"] as ExploreFilter[]).map((item) => (
              <Pressable
                key={item}
          ...
          <View style={styles.sortBlock}>
            ...
            <View style={styles.sortRow}>
              {(["Nome", "Cidade", "Categoria"] as ExploreSort[]).map((item) => (
...
const styles = StyleSheet.create({
  ...
  statsRow: { flexDirection: "row", gap: 8, marginBottom: 11 },
  ...
  searchWrap: { flexDirection: "row", alignItems: "center", borderWidth: 1, borderColor: brand.colors.border, borderRadius: brand.radius.md, backgroundColor: brand.colors.surface, paddingHorizontal: 14, ...brand.shadow },
  ...
  sortBlock: { marginBottom: 12, borderWidth: 1, borderColor: brand.colors.borderSoft, borderRadius: brand.radius.md, backgroundColor: brand.colors.surface, padding: 11 },
  sortLabel: { color: brand.colors.gold, fontSize: 8, fontWeight: "900", letterSpacing: 0.8, marginBottom: 8 },
  sortRow: { flexDirection: "row", gap: 7 },
  sortChip: { flex: 1, alignItems: "center", borderWidth: 1, borderColor: brand.colors.borderSoft, borderRadius: brand.radius.pill, backgroundColor: brand.colors.bgDeep, paddingVertical: 8 },
  sortChipActive: { borderColor: brand.colors.gold, backgroundColor: "#3c2c0d" },
  sortText: { color: brand.colors.mutedStrong, fontSize: 9, fontWeight: "800" },
  sortTextActive: { color: brand.colors.gold },
  ...
  card: { position: "relative", overflow: "hidden", flexDirection: "row", alignItems: "center", gap: 12, marginBottom: 10, borderWidth: 1, borderColor: brand.colors.borderSoft, borderRadius: brand.radius.lg, backgroundColor: brand.colors.surface, padding: 12, ...brand.shadow },
  ...
  verified: { width: 16, height: 16, borderRadius: 8, alignItems: "center", justifyContent: "center", backgroundColor: brand.colors.cyan },
  verifiedText: { color: brand.colors.bgDeep, fontSize: 9, fontWeight: "900" },
  ...
  team: { marginTop: 3, color: brand.colors.muted, fontSize: 10, fontWeight: "700" },
