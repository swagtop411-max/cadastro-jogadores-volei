...
import{collection,deleteField,doc,getDocs,getFirestore,serverTimestamp,writeBatch}from"https://www.gstatic.com/firebasejs/12.1.0/firebase-firestore.js";
...
async function runMigration(){if(!isAdmin())return;const btn=document.getElementById("v11RunMigration"),status=document.getElementById("v11MigrationStatus");if(btn)btn.disabled=true;if(status)status.textContent="Lendo dados e preparando migração...";try{
 const [profiles,configs,athletes,teams,posts,videos,stories,logs,stats]=await Promise.all([rows("perfis"),rows("config_perfis"),rows("atletas"),rows("equipes"),rows("publicacoes"),rows("videos"),rows("stories"),rows("access_logs"),rows("site_stats")]);
 const privacy=new Map(configs.map(d=>[d.id,d.data()?.privado===true]));const ops=[];let moved=0,handles=0,visibility=0,expired=0;
 for(const pdoc of profiles){const p=pdoc.data()||{},handle=handleOf(p,pdoc.id);if(p.handle!==handle){ops.push(batch=>batch.set(pdoc.ref,{handle},{merge:true}));handles++}ops.push(batch=>batch.set(doc(db,"handles",handle),{uid:pdoc.id,handle,atualizadoEm:serverTimestamp()},{merge:true}))}
 for(const d of athletes){const a=d.data()||{},privatePatch={},publicPatch={};let dirty=false;for(const key of sensitiveAthlete){if(a[key]!==undefined){privatePatch[key]=a[key];publicPatch[key]=deleteField();dirty=true}}if(dirty){privatePatch.atualizadoEm=serverTimestamp();privatePatch.origem="migracao-v11";ops.push(batch=>batch.set(doc(db,"atletas",d.id,"privado","dados"),privatePatch,{merge:true}));ops.push(batch=>batch.update(d.ref,publicPatch));moved++}}
 for(const d of teams){const a=d.data()||{},privatePatch={},publicPatch={};let dirty=false;for(const key of sensitiveTeam){if(a[key]!==undefined){privatePatch[key]=a[key];publicPatch[key]=deleteField();dirty=true}}if(dirty){privatePatch.atualizadoEm=serverTimestamp();privatePatch.origem="migracao-v11";ops.push(batch=>batch.set(doc(db,"equipes",d.id,"privado","dados"),privatePatch,{merge:true}));ops.push(batch=>batch.update(d.ref,publicPatch));moved++}}
 for(const group of [posts,videos,stories])for(const d of group){const data=d.data()||{},desired=privacy.get(data.ownerUid)?"privado":"publico";if(data.visibilidade!==desired){ops.push(batch=>batch.set(d.ref,{visibilidade:desired},{merge:true}));visibility++}}
 const now=Date.now();for(const d of [...logs,...stats]){const v=d.data()?.expiraEm,ms=v?.toMillis?.()||0;if(ms&&ms<now){ops.push(batch=>batch.delete(d.ref));expired++}}
...
onAuthStateChanged(auth,user=>{if(text(user?.email).toLowerCase()===ADMIN_EMAIL){if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",addPanel,{once:true});else addPanel()}});
