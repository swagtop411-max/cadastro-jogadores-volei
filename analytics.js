...
mobileAdminStyle.textContent = `
  ...
  @media (max-width:430px) {
    .internal-stats-grid { grid-template-columns:1fr!important; }
  }
`;
...
async function loadInternalAthleteStats() {
  ...
  try {
    ...
    const ativos = atletas.filter(a => a.status !== "inativo").length;
    ...
    const recentes = [...atletas].sort((a,b) => {
      const ta = a.criadoEm?.seconds || 0, tb = b.criadoEm?.seconds || 0;
      return tb-ta;
    }).slice(0,5);

    root.innerHTML = `
      <div class="internal-stats-section">
        <div class="internal-stats-head">
          <div><span class="internal-kicker">DADOS DO BANCO</span><h3>📊 Estatísticas dos atletas</h3><p>Informações calculadas diretamente dos cadastros armazenados no sistema.</p></div>
          <button type="button" id="refreshInternalStats" class="internal-refresh">↻ Atualizar</button>
        </div>
        <div class="internal-stats-grid">
          <div class="internal-kpi"><span>👥</span><strong>${atletas.length}</strong><small>Atletas cadastrados</small></div>
          <div class="internal-kpi"><span>🟢</span><strong>${ativos}</strong><small>Atletas ativos</small></div>
          <div class="internal-kpi"><span>📍</span><strong>${cidades}</strong><small>Cidades representadas</small></div>
          <div class="internal-kpi"><span>⭐</span><strong>${mediaTecnica}</strong><small>Média técnica dos avaliados</small></div>
        </div>
        <div class="internal-stats-columns">
          ${renderStatList("Por posição", topCounts(atletas,"posicao"))}
          ${renderStatList("Por categoria", topCounts(atletas,"categoria"))}
          ${renderStatList("Por cidade", topCounts(atletas,"cidade",8))}
          ${renderStatList("Por equipe atual", topCounts(atletas,"time",8))}
        </div>
        ...
        <div class="internal-footer-actions"><span>🌐 Para dados de visitas e acessos ao site, use o Google Analytics.</span><a href="${GA_PROPERTY}" target="_blank" rel="noopener noreferrer" class="internal-ga-link">Abrir Google Analytics ↗</a></div>
      </div>`;
    ...
    console.error("Estatísticas internas:", error);
    root.dataset.loading = "0";
...
function installInternalStats() {
  ...
  const oldGrid = view.querySelector(".stats-grid");
  const oldLinks = view.querySelector(".stats-links");
  if (oldGrid) oldGrid.style.display = "none";
  if (oldLinks) oldLinks.style.display = "none";
  const root = document.createElement("div");
  root.id = "internalAthleteStats";
  view.querySelector(".stats-panel")?.appendChild(root);
  loadInternalAthleteStats();
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (cadastroLink) cadastroLink.addEventListener("click", () => trackEvent("cadastro_aberto", {origem:"site"}));
  const searchButton = document.getElementById("btnPesquisar");
  if (searchButton) searchButton.addEventListener("click", () => trackEvent("pesquisa_atletas", {origem:"site"}));
  const cadastroForm = document.getElementById("cadastroAtletaForm");
  if (cadastroForm) cadastroForm.addEventListener("submit", () => trackEvent("cadastro_enviado", {origem:"cadastro_atleta"}));
  ...
  const statsTab = document.querySelector('[data-tab="estatisticasView"]');
  if (statsTab) statsTab.addEventListener("click", () => setTimeout(installInternalStats, 0));
